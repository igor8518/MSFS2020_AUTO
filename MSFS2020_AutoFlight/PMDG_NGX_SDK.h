...
struct PMDG_NGX_Data
{
	...
	// ADIRU
	unsigned char	IRS_DisplaySelector;				// Positions 0..4 0x0
	bool			IRS_SysDisplay_R;					// false: L  true: R 0x1
	bool			IRS_annunGPS;						//0x2
	bool			IRS_annunALIGN[2];						//x03
	bool			IRS_annunON_DC[2];						//0x5
	bool			IRS_annunFAULT[2];						//0x7
	bool			IRS_annunDC_FAIL[2];						//09
	unsigned char	IRS_ModeSelector[2];					// 0: OFF  1: ALIGN  2: NAV  3: ATT//0xb

	// PSEU
	bool			WARN_annunPSEU;						//0xd

	// Service Interphone
	bool			COMM_ServiceInterphoneSw;//0xe

	// Lights
	unsigned char	LTS_DomeWhiteSw;					// 0: DIM  1: OFF  2: BRIGHT 0xf

	// Engine
	bool			ENG_EECSwitch[2];//0x10
	bool			ENG_annunREVERSER[2];//0x12
	bool			ENG_annunENGINE_CONTROL[2];//0x14
	bool			ENG_annunALTN[2];//0x16

	// Oxygen
	unsigned char	OXY_Needle;							// Position 0...240 0x18
	bool			OXY_SwNormal;						// true: NORMAL  false: ON 0x19
	bool			OXY_annunPASS_OXY_ON; //0x1a

	// Gear
	bool			GEAR_annunOvhdLEFT; //0x1b
	bool			GEAR_annunOvhdNOSE; //0x1c
	bool			GEAR_annunOvhdRIGHT; //0x1d

	// Flight recorder
	bool			FLTREC_SwNormal;					// true: NORMAL  false: TEST 0x1e
	bool			FLTREC_annunOFF; //0x1f
	...
	// Flight Controls
	unsigned char	FCTL_FltControl_Sw[2];				// 0: STBY/RUD  1: OFF  2: ON //0x20
	...
	bool			FCTL_annunAUTO_SLAT_FAIL;	//0x30
	...
	bool			FUEL_annunENG_VALVE_CLOSED[2]; //0x41
	...
	// APU
	float			APU_EGTNeedle;				// Value //0x6c
	bool			APU_annunMAINT;
	bool			APU_annunLOW_OIL_PRESSURE; 
	bool			APU_annunFAULT; //0x70
	...
	bool			AIR_EquipCoolingSupplyNORM; //0x76
	...
	bool			HYD_annunLOW_PRESS_elec[2]; //0xa1
	...
	unsigned int	AIR_LandAltWindow;	//0xc3	
	...
	bool			LTS_LandingLtFixedSw[2]; //0xd1
	...
	bool			WARN_annunMASTER_CAUTION[2]; //0xe1
	...
	bool			MCP_FDSw[2];
	bool			MCP_ATArmSw;
	unsigned char	MCP_BankLimitSel;					// 0: 10 ... 4: 30   //0x110
	...
	unsigned char	MAIN_DisengageTestSelector[2];			// 0: 1  1: OFF  2: 2 //0x130
	...
	bool			GPWS_GearInhibitSw_NORM; //0x170
	...
	bool			PED_annunParkingBrake; //0x180
	...
	bool			FIRE_annunBOTTLE_DISCHARGE[2]; //0x190
	...
	unsigned char   COMM_Attend_PressCount;				// incremented with each button press //0x1b7
	...
	unsigned char	FMC_V2;								// knots, 0 if not set //0x0d0
	...
	float			FMC_DistanceToDest;					// nm; negative if n/a //0x1e0
	...
	float			AIR_TemperatureNeedle;			// Value - degrees C //0x224
	float			AIR_DuctPressNeedle[2];			// Value - degrees C
	char			ELEC_MeterDisplayTop[13];		// Top line of the display: 3 groups of 4 digits (or symbols) + terminating zero //0x240
	char			ELEC_MeterDisplayBottom[13];	// Bottom line of the display //0x24d
	char			IRS_DisplayLeft[7];				// Left display string, zero terminated //0x25a
	char			IRS_DisplayRight[8];			// Right display string, zero terminated //0x261
	bool			IRS_DisplayShowsDots;			// True if the degrees and decimal dot symbols are shown on the IRS display //0x269
