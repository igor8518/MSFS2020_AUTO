...
void CabinWork::BatterySwitch(DWORD num, DWORD onoff) {
		...
		//emit SendEvent2(KEY_ELECTRICAL_BUS_TO_BUS_CONNECTION_TOGGLE, 6, BUS_LOOKUP_INDEX, 11);
		//emit SendEvent(KEY_ELECTRICAL_BUS_TO_BATTERY_CONNECTION_TOGGLE, 6);
		//SetData(BUS_LOOKUP_INDEX, 10);
		//SetData(BUS_CONNECTION_ON6, 1);
		//if (num == 1) {
	SetDataL(A32NX_OVHD_ELEC_BAT_1_PB_IS_AUTO + num - 1, onoff);
		//}
		//else {
//			SetData(BUS_LOOKUP_INDEX, 11);
			//SetData(BUS_CONNECTION_ON6, onoff);
		//}
}
...
void CabinWork::ParkingBrakeSet(DWORD onoff) {
	if (DataT->AllData.A32NX_PARK_BRAKE_LEVER_POS != onoff) {
		...
		//emit SendEvent(KEY_PARKING_BRAKES, 0);
	}
}
void CabinWork::FlapsSet(DWORD pos) {
	...
	for (int i = GetData(FLAPS_HANDLE_INDEX); i != pos; ) {
		...
		if (i < pos) {
			...
			//SetDataL(A32NX_FLAPS_HANDLE_INDEX, i+1);
			//i++;
		}
		else if (i>pos) {
			emit SendEvent(KEY_FLAPS_UP + i - 1, 0);
...
char* CabinWork::GetTitleCDU() {
	//double a = (data->AllData.MIGO_MCDU_BUFFER_0_0);
	//char* b = (char*)&a;
	//return b;
	return "";
}
...
void CabinWork::PreliminaryCocpitPrep(int* Status)
{
	if (*Status != 2) {
		*Status = 1;
		//WindowsAndDoors(0, 1);
...
void CabinWork::TrustRedAccAlt(double red, double acc) {
	if ((DataT->AllData.AIRLINER_THR_RED_ALT) != red) {
		SetDataL(AIRLINER_THR_RED_ALT, red);
	}
	if ((DataT->AllData.AIRLINER_ACC_ALT) != acc) {
		SetDataL(AIRLINER_ACC_ALT, acc);
	}
}
...
void CabinWork::SetCruise(DWORD alt) {
...
void CabinWork::SpeedMachMode(DWORD mode) {
	/*if ((data->AllData.AP_MANAGED_AIRSPEED) != mode) {
		SetDataL(AP_MANAGED_AIRSPEED, mode);
	}*/
	if (mode) {
...
void CabinWork::AltSet(DWORD alt) {
	if (GetData(AUTOPILOT_ALTITUDE_LOCK_VAR) != alt) {
		alt = int(alt / 100) * 100;
		//bool change = false;
	...
	//	}
		...
		while ((int(GetData(AUTOPILOT_ALTITUDE_LOCK_VAR) / 1000) * 1000) != (int(alt / 1000) * 1000)) {
			currAlt = int(GetData(AUTOPILOT_ALTITUDE_LOCK_VAR) / 1000) * 1000;
			if (alt > (int(GetData(AUTOPILOT_ALTITUDE_LOCK_VAR) / 1000) * 1000)) {
			...
			emit SendEvent(KEY_AP_ALT_VAR_SET_ENGLISH, currAlt);
			while ((int(GetData(AUTOPILOT_ALTITUDE_LOCK_VAR) / 100) * 100) != currAlt);
			//Sleep(100);
		}
		if (alt != (int(GetData(AUTOPILOT_ALTITUDE_LOCK_VAR) / 100) * 100)) {
			SetAltitudeIncrement(0);
			while ((int(GetData(AUTOPILOT_ALTITUDE_LOCK_VAR) / 100) * 100) != (int(alt / 100) * 100)) {
				currAlt = int(GetData(AUTOPILOT_ALTITUDE_LOCK_VAR) / 100) * 100;
				if (alt > (int(GetData(AUTOPILOT_ALTITUDE_LOCK_VAR) / 100) * 100)) {
					currAlt = currAlt + 100;
				}
				else {
...
void CabinWork::CocpitPreparation(int* Status)
{
	if (*Status != 2) {
		*Status = 1;
		Sleep(2000);
		...
		SpeedMachMode(0);
		HdgVsTrkFpa(0);
		//AltSet((DataT->AllData.AIRLINER_CRUISE_ALTITUDE));
...
void CabinWork::BeforeStart(int* Status) {
	*Status = 1;
	Sleep(2000);
	//LoadSheetCheck
	//TakeOffDataCheck
...
void CabinWork::EngineIdleParameters(DWORD num) {
	while (GetData(ENG_N1_RPM1 - 1 + num, "percent over 100") < 0.195);
	Sleep(5000);
}
void CabinWork::EngineStart(int* Status) {
	if (*Status != 2) {
		*Status = 1;
		Sleep(2000);
		FuelPump(2, 1);
...
void CabinWork::AfterStart(int* Status) {
	if (*Status != 2) {
		*Status = 1;
		Sleep(2000);
...
void CabinWork::LandingLight(DWORD onoff) {
	...
	if (GetData(LIGHT_LANDING3) != onoff) {
		emit SendEvent(KEY_LANDING_LIGHTS_TOGGLE, 3);
	}
}
...
void CabinWork::SpeedSel(DWORD speed) {
	while (DataT->GData.AUTOPILOT_AIRSPEED_HOLD_VAR != speed) {
		if (DataT->GData.AUTOPILOT_AIRSPEED_HOLD_VAR < speed) {
			...
			//SendEvent(KEY_AIRSPEED_BUG_SELECT, 1);
			//SendEvent(A320_Neo_FCU_SPEED_INC, 0);
		}
		else {
			SendEvent(A32NX_FCU_SPD_SET, DataT->GData.AUTOPILOT_AIRSPEED_HOLD_VAR - 1);
			//SendEvent(KEY_AIRSPEED_BUG_SELECT, 0);
			//SendEvent(A320_Neo_FCU_SPEED_DEC, 0);
		}
		Sleep(60);
	}
}
...
void CabinWork::BeforeTaxi(int flightLevel, int heading, int* Status) {
	if (*Status != 2) {
		*Status = 1;
		Sleep(2000);
		CabinLight(0);
...
void CabinWork::BeforeTakeoff(int* Status) {
	if (*Status != 2) {
		*Status = 1;
		APUSet(0);
		Sleep(2000);
...
{
	emit SetSetDataSignal(CABINWORK_ID, var1, &val1, var2, &val2, unit1, unit2);
...
void CabinWork::GetDataChange(DWORD var, DWORD sender) {
	if (sender == CABINWORK_ID) {
		SetDataChanged = true;
	}
}
void CabinWork::SetGetDataChange(DWORD var1, DWORD var2, DWORD sender) {
	if (sender == CABINWORK_ID) {
		SetGetDataChanged = true;
	}
}

void CabinWork::SetSetDataChange(DWORD var1, DWORD var2, DWORD sender)
{
	if (sender == CABINWORK_ID) {
